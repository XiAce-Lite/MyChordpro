(function attachChordWikiSetlists(global) {
  /* MyChordpro: クラウド API 無し運用でもローカル CRUD が動くようにする。
   * ストレージキーを mcp_* に分離（他アプリとの衝突回避）。*/
  const STORAGE_KEY = 'mcp_setlists:v1';
  ...
  function enqueueRemoteOperation(operation) {
    if (global.ChordWikiRuntime?.setlistsCloudSyncEnabled === false) {
      return remoteQueue;
    }
