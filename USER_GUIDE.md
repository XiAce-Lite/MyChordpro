# MyChordpro USER GUIDE

このファイルは、**MyChordpro の使い方ガイド**です。  
システム構成や API 仕様は `MyChordpro-Architecture.md` を参照してください。
...
- （ドロップダウンで）`target=song`：曲名 / アーティストを検索
- （ドロップダウンで）`target=tag`：タグを検索
- 通常入力：部分一致
- ダブルコーテーションで括ると、`"完全一致"`：完全一致検索（曲名 / アーティスト検索時）
...
- **表示カスタマイズ**：コードサイズ、**記号サイズ**、**記号縦位置**、**コード行縦位置**、行間、**コード行に|を延伸**、**ブロック内の小節線を揃える** などの見た目調整
- 各ブロックは **▼ / ▶** で縦に折りたたみ可能
...
1. 左側の `Start` / `End` マーカー位置を必要に応じて調整する
   - `End` マーカーは譜面下端より少し下まで動かせます
2. 再生したい時間を設定する
3. `可変スクロール` を必要に応じて切り替える（OFF で等速モード）
4. `Start` を押して再生する
5. 必要なら再生中に速度を微調整する（最大 `3.00x`）
6. `End` マーカーがビューポート内へ100px入った時点で自動停止します
   - （実装上、Endマーカーのtop座標が`window.innerHeight - 100`以下になった時点で停止します）

既定の再生時間付近では、**曲長の自動推定**（iTunes / MusicBrainz / YouTube のいずれかを参照）が 1 回だけ走り、推定に成功すると分・秒入力とステータス表示が更新されます。手動で時間を変えたあとは自動では上書きしません。
...
- トップの **「セットリスト」** から `/setlists.html` を開きます
- リストや曲順は **このブラウザの localStorage のみ**に保存され、**既定ではサーバー（Cosmos）へ同期しません**（`MyChordpro-Architecture.md` §6.8 参照）
- 複数リストの作成・並べ替え・削除はログイン後の画面から行えます
...
- `title`：曲名
- `slug`：URL 用 ID（現在未使用ですが、入力してください。タイトルを自動入力します）
- `artist`：アーティスト名
- `tags`：**1 行 1 タグ**
- `youtube`：**1 行 1 動画**
- `chordPro`：譜面本文
...
- 技術仕様・構成: `MyChordpro-Architecture.md`
- 概要: `README.md`
